...
**Why no map** — Leaflet integration was scoped out of the MVP. Coordinates are stored on every incident; a map panel is a one-dependency addition when bandwidth allows.
...
No AI feature is implemented in the current version. The planned feature was a natural-language fault summary attached to each ticket (call the Claude API to explain the span boundary, confidence reasons, and affected households in plain English). It was deferred because the core localization, ticket lifecycle, and test suite took priority within the available time.

Cost estimate if implemented: one API call per new incident, ≈200–400 input tokens + ≈150 output tokens ≈ $0.001 per incident at current Sonnet pricing. The feature would degrade gracefully: if the model is unavailable, the ticket is created without a summary and the reasons array is shown raw.
